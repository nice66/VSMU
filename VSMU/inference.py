...
def single_case_test(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    print(image.shape)
...
def all_case_test(net, image_list, num_classes=2, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for ith,image_path in enumerate(image_list):
        ...
        prediction, score_map = single_case_test(net, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
...
if __name__ == '__main__':
    data_path = r'/dataset20'
    test_save_path = 'predictions/fif/'
    save_mode_path = 'checkpoint/UNet_seven21/checkpoint.pth'
    net = VSMU(num_classes=4, input_channels=4).cuda()
    net.load_state_dict(torch.load(save_mode_path)['model'], strict=False)
    ...
    avg_metric = all_case_test(net, image_list, num_classes=4,
